...
(use-package evil
  ...
  (progn
    ...
    (defun evil-collection-load-and-init ()
      "Load and initialize evil collection"
      (when (file-exists-p "~/.emacs.d/evil-collection/")
        (add-to-list 'load-path (expand-file-name "~/.emacs.d/evil-collection" user-emacs-directory))
        (when (require 'evil-collection nil t)
          (evil-collection-init))))

    (add-hook 'after-init-hook 'evil-collection-load-and-init)
...
Relative line numbers. Currently disabled, becauseO having them seems a bit redundant with vim easymotions.
#+BEGIN_SRC emacs-lisp
