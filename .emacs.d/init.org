...
(defun eshell/x (&rest args)
  "Makes it so the exit command closes the window as well as the buffer"
  (ignore-errors
    (kill-buffer-and-window)))

(global-set-key (kbd "C-c <C-return>") 'eshell-here)
#+END_SRC
** Always force UTF8
#+BEGIN_SRC emacs-lisp
...
(use-package emms
  ...
  :init
  (progn
    ;;I would pretty much never want to add directory without browsing it recursively
    (defalias 'emms-add-directory 'emms-add-directory-tree)

    ;;Custom key bindings. My laptop has no media next/previous so home and end are used instead
    (global-set-key (kbd "<XF86AudioPlay>") 'emms-pause)
    (global-set-key (kbd "<home>") 'emms-previous)
    (global-set-key (kbd "<end>") 'emms-next)
    ;;Disable evin in emms
